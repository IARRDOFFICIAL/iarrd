...
  
  const handleSubscribe = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setSubscribeStatus(''); // Clear status message on new attempt
    ...
    try {
      ...
      if (response.ok) {
        setSubscribeStatus(data.message); // Get success message from the backend
        setEmail(''); // Clear the input field on success
      } else {
        setSubscribeStatus(data.message || 'Something went wrong. Please try again.'); // Show error message from backend
      }
    } catch (error) {
      console.error("Error submitting form:", error);
      setSubscribeStatus("Network error. Could not connect to the server.");
    }
  };
